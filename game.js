...
class Game {
    ...
    initLevel() {
        ...
        this.levelStartTime = Date.now();
        this.pausedTime = 0; // Reset paused time for new level
        this.particles = [];
    ...
    levelComplete() {
        // Calculate time bonus (excluding paused time)
        const timeElapsed = Math.floor((Date.now() - this.levelStartTime - this.pausedTime) / 1000);
    ...
    drawPauseOverlay() {
        ...
        this.ctx.fillText('Arrow Keys: Move', this.canvas.width / 2, this.canvas.height / 2 + 80);
        this.ctx.fillText('Space: Attack', this.canvas.width / 2, this.canvas.height / 2 + 110);
        this.ctx.restore();
    }
